...
analyze <- function(path) ##path is a string containing the path of the folder containing run_analysis.R
{
  ...
  colsData <- ncol(data) ##rather than hard coding ahead, I'll use these variables
  rowsData <- nrow(data)
  ...
  ##the tidyData will be returned which can be written to disk by the user using write.table(tidyData,file="tidyData.txt",row.name = FALSE)
}
